...
{
    #[init]
    fn init(&self, fee: BigUint, token: MoaOrDctTokenIdentifier) {
        self.whitelist_fee_token(fee, token);
    }

    #[endpoint(whitelistFeeToken)]
    #[only_owner]
    fn whitelist_fee_token(&self, fee: BigUint, token: MoaOrDctTokenIdentifier) {
    ...
    #[only_owner]
    fn blacklist_fee_token(&self, token: MoaOrDctTokenIdentifier) {
    ...
    fn claim_fees(&self) {
        ...
        for token in fee_tokens {
            ...
            if token == MoaOrDctTokenIdentifier::moa() {
                self.send().direct_moa(&caller_address, &fee);
            } else {
