...
The basic idea of Dharitri Digital Cash is that ONE link can hold information (DCT tokens, MOA) on chain, this link can be sent from one person to another, there is no need to hold any wallet to receive, consume and send it forward to another person.
...
In order to fund one should first call `deposit_fees` depositing the fee funds as `MOA` within the contract. Only after, if the fees cover the transfer of the certain number of tokens, it is possible to deposit the funds, making them available for claiming or forwarding.
