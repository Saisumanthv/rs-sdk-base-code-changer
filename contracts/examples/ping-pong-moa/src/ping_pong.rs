...
pub trait PingPong {
    ...
    /// `ping_amount` - the exact MOA amounf that needs to be sent when `ping`-ing.
    ...
    /// User sends some MOA to be locked in the contract for a period of time.
        ...
        match user_status {
            ...
            UserStatus::Registered => {
                ...
                if let Some(user_address) = self.user_mapper().get_user_address(user_id) {
                    self.send()
                        .direct_moa(&user_address, &self.ping_amount().get());
                    Result::Ok(())
                } else {
