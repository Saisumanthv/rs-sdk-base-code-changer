...
pub trait Erc1155Marketplace {
    ...
    fn bid(&self, type_id: BigUint, nft_id: BigUint) {
        let (payment_token, payment) = self.call_value().moa_or_single_fungible_dct();
        require!(
    ...
    fn add_claimable_funds(&self, token_identifier: &MoaOrDctTokenIdentifier, amount: &BigUint) {
    ...
    fn clear_claimable_funds(&self, token_identifier: &MoaOrDctTokenIdentifier) {
        let mut mapper = self.get_claimable_funds_mapper();
    ...
    fn get_claimable_funds_mapper(&self) -> MapMapper<MoaOrDctTokenIdentifier, BigUint>;
