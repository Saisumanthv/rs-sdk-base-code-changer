use crate::{
    tx_execution::{builtin_function_names::DCT_NFT_ADD_URI_FUNC_NAME, BlockchainVMRef},
...
pub struct DCTNftAddUri;

impl BuiltinFunction for DCTNftAddUri {
    fn name(&self) -> &str {
        ...
        F: FnOnce(),
    {
        if tx_input.args.len() < 3 {
            let err_result = TxResult::from_vm_error("DCTNFTAddURI expects at least 3 arguments");
        ...
        let dct_nft_create_log = TxLog {
            ...
            endpoint: DCT_NFT_ADD_URI_FUNC_NAME.into(),
            topics,
            data: vec![],
        };
