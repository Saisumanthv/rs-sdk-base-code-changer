...
pub(crate) fn mandos_to_erdrs_address(mandos_address: &AddressValue) -> MoarsAddress {
    let bytes = mandos_address.value.as_array();
    MoarsAddress::from_bytes(*bytes)
}

pub(crate) fn address_h256_to_erdrs(address: &Address) -> MoarsAddress {
    let bytes = address.as_array();
    MoarsAddress::from_bytes(*bytes)
}

pub(crate) fn erdrs_address_to_h256(erdrs_address: MoarsAddress) -> Address {
