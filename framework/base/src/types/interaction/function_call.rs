...
use crate::{
    ...
    api::{
        ManagedTypeApi, DCT_MULTI_TRANSFER_FUNC_NAME, DCT_NFT_TRANSFER_FUNC_NAME,
        DCT_TRANSFER_FUNC_NAME,
    },
    formatter::SCLowerHex,
    types::{
        DctTokenPayment, ManagedAddress, ManagedBuffer, ManagedBufferCachedBuilder, ManagedVec,
        MultiValueEncoded,
    },
};
...
where
    Api: ManagedTypeApi,
{
    /// Constructs `DCTTransfer` builtin function call.
    pub(super) fn convert_to_single_transfer_fungible_call(
        self,
        payment: DctTokenPayment<Api>,
    ) -> FunctionCall<Api> {
        FunctionCall::new(DCT_TRANSFER_FUNC_NAME)
            .argument(&payment.token_identifier)
            .argument(&payment.amount)
            .argument(&self)
    }

    /// Constructs `DCTNFTTransfer` builtin function call.
    ///
    ...
    pub(super) fn convert_to_single_transfer_nft_call(
        ...
        FunctionCall::new(DCT_NFT_TRANSFER_FUNC_NAME)
            .argument(&payment.token_identifier)
    ...
    /// Constructs `MultiDCTNFTTransfer` builtin function call.
    pub(super) fn convert_to_multi_transfer_dct_call(
        self,
        to: &ManagedAddress<Api>,
        payments: ManagedVec<Api, DctTokenPayment<Api>>,
    ) -> FunctionCall<Api> {
        let mut result = FunctionCall::new(DCT_MULTI_TRANSFER_FUNC_NAME)
            .argument(&to)
